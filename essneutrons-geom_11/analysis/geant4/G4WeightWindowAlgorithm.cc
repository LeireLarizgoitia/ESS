                                   ...
                                   G4double lowerWeightBound) const
{
  ...
  if (init_w > upperWeight)
  {
    ...
    G4int split_i = 0;
    if(temp_wi_ws > fMaxNumberOfSplits) {
      split_i = 1; //fMaxNumberOfSplits;
      G4cout << "WARNING: splitting " << temp_wi_ws 
	     << " > max. number of splits. Not splitting track."
	     << G4endl << std::flush;
    }
    else {
      split_i = static_cast<G4int>(temp_wi_ws);
    ...
    if(wi_ws < 0 || split_i < 0) {
      ...
      os << "Negative! With "
         ...
         << "\nsplit_i = " << split_i
         << "\ntemp_wi_ws = " << temp_wi_ws
         << "\nwi_ws = " << wi_ws
         << "\n---------";
      G4String msg = os.str();

      G4Exception("G4WeightWindowAlgorithm::Calculate()",
